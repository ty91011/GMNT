				...
				$tour = str_replace("'", "''", $_REQUEST['tour']);
				$query = "SELECT e.*, coalesce(num, 0) as num, coalesce(avgPrice, 'NA') avgPrice, coalesce(skybox, 0) skybox, coalesce(tickets, 0) tickets, coalesce(rows, 0) rows, coalesce(platinum,0) platinum, case when datetime > NOW() then 1 else 0 end as future, case when datetime > NOW() and datetime < date_add(NOW(), interval 4 day) then 1 else 0 end as critical
						    FROM events e 
							    left join 
							    (
							    ...
							    ON e.tmId = t.tmId
						    WHERE e.datetime > NOW() and e.tour='$tour'
						    GROUP BY e.id
